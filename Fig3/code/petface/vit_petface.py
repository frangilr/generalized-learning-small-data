...
log_key = {0: "zero", 1: "one", 2:"two", 3:"three", 4:"four", 5:"five"}
...
def main():
    ...
    for epoch in range(epochs+1):
        ...
        all_train_correct.append(100 * correct / len(train_loader.dataset))
    ...
    df = pd.DataFrame(
        {
            "train_acc": all_train_correct,
            "test_acc": all_test,
        }
    )
    df["dataset"] = "lumpy" if lumpy else "non-lumpy"
    df["seed"] = _seed

    df.to_csv(f"runs/{"lumpy" if lumpy else "non-lumpy"}_s{_seed}.csv", index=False)
